...
proba = np.zeros(19)
for i in range(0, 19):
    proba[i] = np.sum(data >= threshold[i]) / np.shape(data)[0]
plt.figure(3, figsize=(12, 10))
plt.plot(np.log10(threshold), np.log10(proba), 'k-')
plt.xlabel('Nb of packets per 10 ms', fontsize=24)
plt.ylabel('Probability', fontsize=24)
xticks = np.array([1, 2, 5, 10, 19])
xticks_labels = []
for x in xticks:
    xticks_labels.append('{:d}'.format(x))
yticks = np.array([0.0001, 0.0003, 0.001, 0.003, 0.01, 0.03, 0.1, 0.3])
yticks_labels = []
for y in yticks:
    yticks_labels.append('{:.2e}'.format(y))
plt.xticks(np.log10(xticks), xticks_labels)
...
# Plot fraction of the overall workload
threshold = np.arange(1, 20)
fraction = np.zeros(19)
quantile = np.zeros(19)
for i in range(0, 19):
    fraction[i] = np.sum(data[data >= threshold[i]]) / np.sum(data)
    quantile[i] = np.sum(data <= threshold[i]) / np.shape(data)[0]
plt.figure(4, figsize=(12, 10))
plt.plot(np.log10(quantile), np.log10(fraction), 'k-')
plt.xlabel('Quantile', fontsize=24)
plt.ylabel('Fraction of total workload', fontsize=24)
xticks = np.array([0.7, 0.8, 0.9, 0.99])
xticks_labels = []
for x in xticks:
    xticks_labels.append('{:.2f}'.format(x))
yticks = np.array([0.003, 0.01, 0.03, 0.1, 0.3, 1.0])
yticks_labels = []
for y in yticks:
    yticks_labels.append('{:.2e}'.format(y))
plt.xticks(np.log10(xticks), xticks_labels)
plt.yticks(np.log10(yticks), yticks_labels)
plt.title('Distribution of workload', fontsize=24)
plt.tight_layout()
plt.savefig('figures/Figure4.eps', format='eps')
plt.close(4)
